...
export default function IPManager({ closeDialog }: IPManagerProps) {
  ...
  const handleAddIPRange = () => {
    if (!newIpRange.trim()) {
      toast.error("بازه IP نمی‌تواند خالی باشد");
      return;
    }
    ...
    if (
      ...
      toast.error(
        "فرمت بازه IP نامعتبر است. از CIDR استفاده کنید (مثلاً 192.168.1.0/24)",
      );
      return;
    }
    ...
    if (
      ...
      toast.error("این بازه IP از قبل وجود دارد");
      return;
    }
  ...
  const handleSubmitIPRanges = async () => {
    if (ipRanges.length === 0) {
      toast.error("بازه IP برای ثبت وجود ندارد");
      return;
    }
    ...
    try {
      ...
      toast.success("بازه‌های IP گوگل ذخیره شد", {
        description: `Successfully saved ${ipRanges.length} IP ranges`,
      });
    } catch (error) {
      toast.error("ذخیره بازه‌های IP ناموفق بود", {
      ...
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[380px]">
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-medium dark:text-white">
            افزودن بازه IP تأییدشده Google
          </h3>
          <div className="flex gap-2">
            <Input
              placeholder="بازه IP را وارد کنید (فرمت CIDR)"
              value={newIpRange}
          ...
          <div className="py-2 bg-neutral-100 dark:bg-brand-dark dark:text-white/50 px-2 rounded-md mt-4">
            <p className="text-xs text-muted-foreground">
              بازه‌های IP تأییدشده Google را با نُماد CIDR اضافه کنید (مثلاً 64.233.160.0/19). این بازه‌ها برای شناسایی کراولرهای معتبر Google استفاده می‌شوند.
            </p>
            <p className="text-xs text-muted-foreground">
              Please refer to{" "}
              <a
        ...
        >
          {isSubmitting ? (
            <>
              ...
              <span>در حال ذخیره بازه‌های IP...</span>
            </>
          ) : (
            <>
              <Save className="h-4 w-4" />
              <span>ذخیره بازه‌های IP گوگل</span>
            </>
          )}
        </Button>
      </CardFooter>
