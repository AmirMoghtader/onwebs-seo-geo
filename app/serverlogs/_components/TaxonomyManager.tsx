      ...
      <div className="flex items-center gap-2 flex-1">
        {isEditing ? (
          ...
          <span
            ...
            title="برای ویرایش مسیر کلیک کنید"
          >
            {pathConfig.path}
          </span>
        )}
...
function Segment({ taxonomy, onUpdate, onRemove }) {
  ...
  const handleAddPath = () => {
    ...
    if (pathExists) {
      toast.error("این مسیر از قبل در این Segment وجود دارد");
      return;
    }
  ...
  const handleNameUpdate = () => {
    ...
    if (!cleanName) {
      toast.error("نام Segment نمی‌تواند خالی باشد");
  ...
  return (
    <div className="border dark:border-slate-700 rounded-md p-3 space-y-2 dark:text-white transition-all">
        ...
        <div className="flex items-center gap-2 flex-1 mr-2">
          ...
          {isEditingName ? (
            ...
            <span
              ...
              title="برای تغییر نام Segment کلیک کنید"
            >
              {taxonomy.name}
            </span>
          )}
      ...
      <div className="flex gap-2 pt-2 pl-2">
        <Input
          placeholder="یک مسیر جدید اضافه کنید (مثلاً /blog/)"
          value={newPath}
...
export default function TaxonomyManager({ closeDialog }) {
  ...
  const handleAddSegment = () => {
    ...
    if (
      ...
      toast.error("Segmentی با این نام از قبل وجود دارد");
      return;
    }
  ...
  const handleSubmitTaxonomies = async () => {
    ...
    try {
      ...
      toast.success("Segmentها در دیتابیس ذخیره شدند");
    } catch (error) {
      toast.error("ذخیره Segmentها ناموفق بود");
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Input
              placeholder="نام Segment جدید (مثلاً Blog)"
