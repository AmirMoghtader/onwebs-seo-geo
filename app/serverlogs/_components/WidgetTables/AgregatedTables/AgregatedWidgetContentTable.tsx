...
> = ({ type, title, segment = "all" }) => {
  ...
  const exportCSV = async () => {
    try {
      ...
      toast.error("خروجی گرفتن CSV ناموفق بود");
    } finally {
      setLoading(false);
    }
  };
    ...
    if (key === "verified") {
      return val ? (
        <Badge className="bg-blue-600 text-white border-blue-200 gap-1 text-[10px]">
          <ShieldCheck className="w-3 h-3" /> تأییدشده
        </Badge>
      ) : (
        <Badge
          variant="outline"
        ...
        >
          <ShieldAlert className="w-3 h-3" /> تأییدنشده
        </Badge>
      );
    }
    if (key === "status") {
        ...
        <Pagination className="text-xs w-auto mx-0 pt-2">
          <PaginationContent>
            ...
            <div className="flex items-center gap-1 mx-2">
              <span className="text-xs dark:text-white/50">صفحه</span>
              <Badge
                variant="outline"
