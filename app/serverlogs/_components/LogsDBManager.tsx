...
export default function LogsDBManager({ closeDialog, dbLogs }: any) {
  ...
  const handleRemoveAllLogs = () => {
    try {
      ...
      // setSaveLogs(false);
      handleRefreshLogs();
      toast.success("همه لاگ‌ها از دیتابیس حذف شدند");
    } catch (error) {
      console.error(error);
      toast.error(error);
    }
  ...
  const handleDeleteLog = async (id: string) => {
    try {
      ...
      console.error(error);
      toast.error("حذف لاگ ناموفق بود");
    }
  };
  ...
  const handleDisplayLogs = async () => {
    try {
      ...
      console.log(data);
      toast.success("لاگ‌ها از دیتابیس بازیابی شدند");
    } catch (error) {
      console.error(error);
      toast.error(error);
    }
                ...
                <div>
                  ...
                  <div className="bg-gray-200 dark:bg-brand-dark dark:text-white p-2 rounded-md mb-3">
                    <span className=" text-xs leading-none">
                      به‌طور پیش‌فرض لاگ‌ها در دیتابیس ذخیره نمی‌شوند. می‌توانید این گزینه را فعال کنید تا لاگ‌ها روی دستگاه شما ذخیره و به‌صورت افزایشی بارگذاری شوند.
                    </span>
                  </div>
                  <div className="space-y-4 p-4 border rounded-md bg-muted h-[360px] dark:border-brand-dark">
                    ...
                    <p className="text-xs text-muted-foreground dark:text-white/50">
                      در صورت فعال بودن، لاگ‌ها به‌صورت افزایشی به دیتابیس محلی شما اضافه می‌شوند.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="border dark:border-brand-dark dark:border-brand rounded-lg h-[460px] overflow-y-auto">
                  {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400">
                      ...
                      <p className="text-xs">هیچ لاگی موجود نیست.</p>
                      <p className="text-xs">
                        ذخیره‌سازی در DB را فعال کنید و چند لاگ را اسکن کنید.
                      </p>
                    </div>
                  )}
                </div>
