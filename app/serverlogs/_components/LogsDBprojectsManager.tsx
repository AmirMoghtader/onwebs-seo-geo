...
export default function ProjectsDBManager({ closeDialog, dbProjects }) {
  ...
  const handleCreateProject = useCallback(
    async (projectName: string) => {
      if (!projectName.trim()) {
        toast.error("لطفاً یک نام پروژه وارد کنید");
        return;
      }
      ...
      try {
        ...
        toast.error("ساخت پروژه ناموفق بود");
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const processSingleLog = useCallback(
    async (
      ...
      try {
        ...
        toast.success("لاگ با موفقیت پردازش شد");
      } catch (err) {
        console.error("Processing failed:", err);
        setIsProcessingLogs(false);
        setActiveProject(null);
                ...
                <div className="border dark:border-brand-dark dark:border-brand rounded-lg h-[29rem] overflow-y-auto">
                  {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400">
                      ...
                      <p className="text-xs">هیچ پروژه‌ای با لاگ تخصیص‌داده‌شده وجود ندارد.</p>
                      <p className="text-xs px-4 text-center">
                        ذخیره‌سازی لاگ را فعال کنید و یک پروژه جدید بسازید یا لاگ‌های یک پروژه موجود را تحلیل کنید.
                      </p>
                    </div>
                  )}
                </div>
