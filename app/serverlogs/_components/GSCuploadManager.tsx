  ...
  const handleFileUpload = useCallback(
    async (event) => {
      ...
      if (!file.name.endsWith(".xlsx") && !file.name.endsWith(".csv")) {
        toast.error("لطفاً یک فایل ‎.xlsx یا ‎.csv بارگذاری کنید");
        return;
      }
      ...
      try {
        ...
        if ("Pages" in wb.Sheets) {
          toast.success(
            "فایل Google Sheets شناسایی شد. لطفاً یک sheet برای پردازش انتخاب کنید",
          );
        } else {
          toast.error("در این فایل داده‌ی Pages پیدا نشد");
        }
  ...
  const handleProcessFile = useCallback(async () => {
    ...
    try {
      ...
      if (selectedSheet !== "Pages") {
        toast.error(
          "لطفاً Sheet درست را انتخاب کنید. وجود «Pages» الزامی است",
        );
        return;
      }
      ...
      toast.success("فایل با موفقیت پردازش و بارگذاری شد");
    } catch (error) {
              ...
              {filePreview?.data ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-3">
                  ...
                  <p className="text-sm dark:text-white/40">
                    برای پیش‌نمایش داده، یک فایل و Sheet انتخاب کنید
                  </p>
                </div>
              )}
            </div>
