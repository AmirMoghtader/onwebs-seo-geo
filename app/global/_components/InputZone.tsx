...
const InputZone = ({ handleDomainCrawl }: InputZoneProps) => {
  ...
  // ⌘↩ from the native File menu starts a crawl. The URL lives in this
  // component's state, so the menu asks it to fire rather than duplicating
  // that state somewhere it would drift out of sync.
  useEffect(() => {
    const start = () => handleButtonCrawl();
    window.addEventListener("onwebs:startCrawl", start);
    return () => window.removeEventListener("onwebs:startCrawl", start);
  });
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[2000] h-11 border-b bg-white/95 dark:bg-brand-darker/98 backdrop-blur-md flex items-center px-4 border-gray-100 dark:border-brand-dark transition-all duration-300 shadow-sm dark:bg-brand-darker">
      <MenuDrawer />
