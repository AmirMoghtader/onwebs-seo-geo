...
export default function Duplicates() {
  ...
  const handleOpenUrl = async (url: string) => {
    try {
      ...
      toast.error("باز کردن URL در مرورگر ناموفق بود");
    }
  };
  ...
  const handleCopyUrl = async (url: string) => {
    try {
      ...
      toast.error("کپی URL ناموفق بود");
    }
  };
  ...
  const fetchDuplicateContent = async () => {
    ...
    try {
      ...
      toast.error("اجرای تحلیل محتوای تکراری ناموفق بود");
    } finally {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 max-w-lg">
              صفحاتی را که Headingهای یکسان یا محتوای تقریباً یکسان دارند، بر پایه fingerprintهای ثبت‌شده در زمان کراول خوشه‌بندی می‌کند. فقط با درخواست شما اجرا می‌شود — هیچ چیزی خودکار محاسبه نمی‌شود.
            </p>
          </div>
          <button
        ...
        {!duplicateReport ? (
          ...
          <div className="py-14 flex flex-col items-center gap-3 text-center">
            ...
            <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">
              بررسی محتوای تکراری غیرفعال است
            </span>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 max-w-sm">
              ...
              <span className="font-semibold">
                تنظیمات → Crawler → بررسی محتوای تکراری
              </span>{" "}
          ...
          <div className="py-14 flex flex-col items-center gap-2 text-center">
            ...
            <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">
              هیچ صفحه تکراری یا نزدیک‌به‌تکراری پیدا نشد
            </span>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 max-w-sm">
              اگر این کراول پیش از فعال‌سازی این بررسی انجام شده باشد، صفحات هنوز fingerprint ندارند — کراول را دوباره اجرا و سپس تحلیل کنید.
            </p>
          </div>
        ) : (
