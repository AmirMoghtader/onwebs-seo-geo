    ...
    return (
      <div className="h-64 flex flex-col justify-center items-center w-full text-center px-8">
        ...
        <span className="text-xs font-medium text-gray-500 dark:text-gray-400">
          برای دیدن توصیه‌های دقیق بهینه‌سازی، یک مشکل را از فهرست بالا انتخاب کنید.
        </span>
      </div>
    );
  }
        ...
        {fixDetail ? (
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="flex flex-wrap gap-2 ">
                  {fixDetail.links.map((link, index) => (
                    ...
                    >
                      {/* Google favicon fetch removed — it announced every
                          reference link's hostname to google.com. */}
                      Reference {index + 1}
                    </a>
                  ))}
                </div>
          ...
          <div className="p-4 rounded-lg bg-amber-50 dark:bg-amber-500/5 border border-amber-100 dark:border-amber-500/20 text-center">
            <p className="text-xs text-amber-700 dark:text-amber-400">
              جزئیات فنی این مشکل هنوز در حال تولید است. لطفاً کمی بعد دوباره سر بزنید.
            </p>
          </div>
        )}
