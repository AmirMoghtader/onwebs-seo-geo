...
function computeStats(data: any[]) {
  ...
  const c3 = data.filter((p) => { const s = p?.status_code || 0; return s >= 300 && s < 400; }).length;
...
// ─── Stat tile ────────────────────────────────────────────────────────────────
const StatTile = memo(({ label, value, paletteKey, onClick, active }: {
  label: string;
  value: number;
  paletteKey: PaletteKey;
  onClick?: () => void;
  ...
  return (
    <div
      ...
      title={onClick ? `نمایش فقط ${label} در جدول` : undefined}
      className={`flex flex-col items-center justify-center gap-1.5 py-2.5 px-2 rounded ${
        onClick ? "cursor-pointer transition-transform hover:scale-[1.03]" : ""
      }`}
      style={{
        background: active
          ? `${theme.color}22`
          : isDark ? "rgba(57,57,58,0.35)" : "rgba(255,255,255,0.7)",
        border: `1px solid ${active ? theme.color : isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.08)"}`,
      }}
    >
      <span
...
function OverviewChart() {
  ...
  // Clicking a tile narrows the main crawl table to those rows. Clicking the
  // active tile again clears it, so a tile doubles as its own toggle.
  ...
  const applyFilter = useCallback(
    (kind: string, label: string) => {
      setTableFilter(tableFilter?.kind === kind ? null : { kind, label });
    },
    [tableFilter, setTableFilter],
  );

  const display3xx = finalCrawlStats?.status_3xx ?? failed3xx;
  ...
  return (
    <DarkCtx.Provider value={isDark}>
      <div className="flex flex-col h-full px-4 py-4 overflow-hidden" style={cardStyle}>
        ...
        {/* Primary counter. min-h is its own content height, not a comfortable
            reading size: anything larger is height taken from the tiles below,
            which are the part that actually disappears when the pane is short. */}
        <div
          className="flex flex-col items-center justify-center flex-1 mb-3 rounded shrink-0 min-h-[84px]"
        ...
        {/* Stat tiles. The pane is user-resizable and its height is whatever is
            left after the top pane, so at the default 50/50 split the third row
            (5XX / غیرقابل ایندکس) fell off the bottom edge and the footer with
            it. This wrapper yields its height before the counter or the footer
            do and scrolls what no longer fits, so every tile stays reachable. */}
        <div
          className="min-h-0 shrink overflow-y-auto overflow-x-hidden -mx-1 px-1 mb-3"
          style={{ scrollbarWidth: "thin" }}
        >
          <div className="grid grid-cols-2 gap-3 pb-1">
            <StatTile label="کل لینک‌ها" value={displayTotalLinks} paletteKey="green" />
            <StatTile
              label="3XX"
              value={display3xx}
              paletteKey="cyan"
              active={tableFilter?.kind === "3xx"}
              onClick={() => applyFilter("3xx", "3XX")}
            />
            <StatTile
              label="4XX"
              value={display4xx}
              paletteKey="orange"
              active={tableFilter?.kind === "4xx"}
              onClick={() => applyFilter("4xx", "4XX")}
            />
            <StatTile
              label="5XX"
              value={display5xx}
              paletteKey="red"
              active={tableFilter?.kind === "5xx"}
              onClick={() => applyFilter("5xx", "5XX")}
            />
            <StatTile
              label="غیرقابل ایندکس"
              value={displayNonIndexable}
              paletteKey="violet"
              active={tableFilter?.kind === "nonIndexable"}
              onClick={() => applyFilter("nonIndexable", "غیرقابل ایندکس")}
            />
          </div>
        </div>
