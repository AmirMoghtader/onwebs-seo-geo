...
const DeepCrawlQueryContextMenu: React.FC<DeepCrawlQueryContextMenuProps> = ({
  ...
  const handleCopy = useCallback((query: string) => {
    ...
    toast.success("Query در کلیپ‌بورد کپی شد");
    console.log("✅ Copy operation completed");
  }, []);

  const openSearchConsoleUrl = (query: string) => {
    if (!credentials) {
      toast.error("اطلاعات ورود Search Console پیدا نشد");
      return;
    }
  ...
  const handleTrackKeyword = useCallback(
    async (
      ...
      if (!query || query.trim() === "") {
        toast.error("افزودن keyword ممکن نیست: Query الزامی است");
      ...
      if (!allValid) {
        ...
        toast.error("داده نامعتبر شناسایی شد - برای جزئیات console را ببینید");
        return;
      }
      ...
      try {
        ...
        const mainOperation = async () => {
          ...
          toast.success("Keyword به داشبورد Tracking اضافه شد");
        ...
        if (errorMessage.includes("UNIQUE constraint")) {
          toast.error("این keyword از قبل در Tracking وجود دارد");
        } else if (errorMessage.includes("NOT NULL")) {
          toast.error("داده‌های لازم برای tracking کلمه کلیدی ناقص است");
        } else if (errorMessage.includes("Database save timed out")) {
          toast.error("عملیات ذخیره در دیتابیس منقضی شد - backend را بررسی کنید");
        } else if (errorMessage.includes("Sync operation timed out")) {
          toast.error("همگام‌سازی جدول‌ها منقضی شد - backend را بررسی کنید");
        } else if (errorMessage.includes("timed out after 5 seconds")) {
          toast.error("کل عملیات منقضی شد - اتصال backend را بررسی کنید");
        } else {
          toast.error(
            "افزودن keyword به tracking ناموفق بود: " +
