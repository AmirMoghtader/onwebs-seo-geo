...
const IssuesContainer = () => {
  ...
  // Severity first, then volume. Sorting by count alone (the previous
  // behaviour) scattered High-priority issues among Low-priority ones purely
  // because fewer pages happened to be affected, which is what made the list
  // look unordered. Issues with no affected pages sink to the bottom whatever
  // their priority — a High-severity check that found nothing is not news.
  ...
  const issuesArr = useMemo(
    () =>
      ISSUE_REGISTRY.map((def) => {
        ...
        const aEmpty = a.issueCount === 0;
        const bEmpty = b.issueCount === 0;
  ...
  if (!debouncedCrawlData || debouncedCrawlData.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 h-full opacity-50 dark:text-white">
        ...
        <span className="text-sm font-medium">هیچ نتیجه کراولی نیست.</span>
        <span className="text-[10px]">برای دیدن مشکلات یک دامنه را کراول کنید.</span>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-white dark:bg-brand-darker overflow-hidden">
      <div className="p-3 border-b dark:border-brand-dark bg-gray-50/50 dark:bg-brand-dark/20 flex items-center justify-between">
        <div className="flex flex-col">
          <h2 className="text-sm font-bold dark:text-white">کاوشگر مشکلات</h2>
        ...
        <div className="space-y-1.5 pb-4">
          ...
          {issuesArr.every(i => i.issueCount === 0) && (
            <div className="text-center py-12">
              <span className="text-xs text-green-500 font-medium">✅ هیچ مشکلی پیدا نشد! عالی بود.</span>
            </div>
          )}
        </div>
      </div>
