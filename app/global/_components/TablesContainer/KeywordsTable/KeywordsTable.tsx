...
import { getRowValues } from "./tableLayout";
import {
  useTableSort,
  sortIndicator,
  type SortState,
...
const TableRow = memo(
  ({
    ...
    const rowData = useMemo(
      () => getRowValues(row, index, keywordColumns),
...
const KeywordsTable = ({
  ...
  // The accessor is bound to the dynamic keyword columns so the comparator
  // indexes the same cells the rows render.
  const getSortValues = useCallback(
    (row: any, index: number) => getRowValues(row, index, keywordColumns),
    [keywordColumns],
  );
  ...
  const handleDownload = useCallback(async () => {
    ...
    if (rows.length > 1000) {
      toast.info(
        "داده‌ی بسیار حجیم شناسایی شد. خروجی مستقیم از دیتابیس...",
      );
      ...
      try {
        ...
        toast.error("ساخت فایل Excel ناموفق بود.");
      } finally {
        setIsGeneratingExcel(false);
      }
    }
