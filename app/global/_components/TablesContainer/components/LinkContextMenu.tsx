...
const LinkContextMenu: React.FC<LinkContextMenuProps> = ({
  ...
  const handleCopyAnchor = () => {
    ...
    toast.success("Anchor text در کلیپ‌بورد کپی شد");
  };
  ...
  const handleViewInTable = () => {
    ...
    toast.success("صفحه در جدول کراول انتخاب شد");
  };
  ...
  const fetchQueries = async () => {
    ...
    try {
      ...
      if (Array.isArray(result) && result.length > 0) {
        ...
        toast.info("هیچ Query‌ای در GSC برای این URL پیدا نشد");
      }
    } catch (error) {
      console.error("Error fetching queries:", error);
      toast.error("دریافت Queryها ناموفق بود");
    }
  };
