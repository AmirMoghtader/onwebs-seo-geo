...
const LinksTable = ({
  ...
  // Selecting a row here has to populate the Details pane too. Previously this
  // only moved the highlight, which is why clicking a URL in the داخلی/خارجی
  // tabs left Details empty. Note these rows carry the address in `link`
  // (mapped from the backend's link objects), not `url` like the HTML table.
  const { selectURL } = useDataActions();

  // sortedRows is declared further down (it derives from filteredRows), so the
  // handler reads it through a ref — naming it in the dep array here would
  // evaluate the binding before its initialiser and throw.
  ...
  const handleRowClick = useCallback(
    (rowIndex: number) => {
      ...
      const target = sortedRowsRef.current[rowIndex];
      const address = target?.link || target?.url || target?.page;
      if (address) {
        selectURL(address);
      }
    },
  ...
  // Keeps handleRowClick resolving the clicked index against the order the
  // user is actually looking at, not the pre-sort order.
    ...
    try {
      if (tabName === "Internal Links" || tabName === "All Links") {
        ...
        const filePath = await save({
          ...
          defaultPath: `Onwebs SEO & GEO-InternalLinks.xlsx`,
        });
        if (filePath) {
          await writeFile(filePath, new Uint8Array(internalFileBuffer as any));
          toast.success("خروجی Excel لینک‌های داخلی کامل شد!");
        }
      }
      ...
      if (tabName === "External Links" || tabName === "All Links") {
        ...
        const externalFilePath = await save({
          filters: [{ name: "Excel File", extensions: ["xlsx"] }],
          defaultPath: `Onwebs SEO & GEO-ExternalLinks.xlsx`,
        });
        if (externalFilePath) {
          await writeFile(
            externalFilePath,
          ...
          toast.success("خروجی Excel لینک‌های خارجی کامل شد!");
        }
      }
