...
>(({ url, label, isPage, onFilterByFolder, children, ...others }, ref) => {
  ...
  const handleCopyURL = () => {
    if (url) {
      ...
      toast.error("برای این پوشه URL موجود نیست");
    }
  };

  const handleCopyPath = () => {
    navigator.clipboard.writeText(label);
    toast.success("مسیر در کلیپ‌بورد کپی شد");
  };
  ...
  const handleViewInTable = () => {
    if (url) {
      ...
      toast.success("صفحه در جدول HTML انتخاب شد");
    } else {
      toast.error("برای این پوشه URL موجود نیست");
    }
  };
  ...
  const handleAnalyze = () => {
    if (url) {
      selectURL(url);
      setDeepCrawlTab("crawledPages");
      toast.success("تحلیل در تب جزئیات");
    } else {
      toast.error("داده جزئی برای این URL پیدا نشد");
    }
  };
