...
function MenuDrawer() {
  ...
  async function fullReset() {
    try {
      ...
      // Optionally: toast.success("بازنشانی با موفقیت انجام شد");
      localStorage.clear();
      toast.success("بازنشانی کامل! Onwebs SEO & GEO تا ۳ ثانیه دیگر دوباره بارگذاری می‌شود");
      ...
      toast.error("بازنشانی Onwebs SEO & GEO ناموفق بود. لطفاً دوباره تلاش کنید.");
    }
  }
    ...
    const handleKeyDown = (event: any) => {
      ...
      if (event.ctrlKey && event.key === "/") {
        try {
          ...
          toast.success("cache برنامه Onwebs SEO & GEO پاک شد");
        } catch (error) {
          console.error("Clear failed:", error);
        }
      }

      // Reset Onwebs SEO & GEO with ctrl + shift + "/"
  ...
  return (
    <>
      ...
      >
        {/* The wordmark already contains the Anubis mark, so the separate
            icon.png that used to sit beside it was a second copy of the same
            logo. One image, larger. */}
        <img
          src="onwebs-wordmark.png"
          className="object-contain h-auto w-44"
          alt="Onwebs SEO & GEO"
        />
      </a>
    </>
