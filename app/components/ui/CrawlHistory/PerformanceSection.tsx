...
const PerformanceSection: React.FC<PerformanceSectionProps> = ({
  ...
  const handleDownloadXLSX = async () => {
    try {
      ...
      const path = await save({
        defaultPath: "Onwebs SEO & GEO Technical Table.csv",
        filters: [
          {
            name: "CSV Files",
            extensions: ["csv"],
      ...
      if (path) {
        ...
        toast.success("فایل CSV با موفقیت ذخیره شد!");
      } else {
        console.log("File save canceled by the user.");
        toast.info("ذخیره فایل توسط کاربر لغو شد.");
      }
    } catch (error) {
      console.error("Error during download process:", error);
      toast.error("خطا در ذخیره فایل CSV. لطفاً دوباره تلاش کنید.");
    }
  };
  ...
  const handleClearTable = async () => {
    try {
      ...
      toast("جدول‌ها پاک شدند");
    } catch (error) {
      console.error("Error clearing tables:", error);
      toast("خطا در پاک کردن جدول‌ها");
    }
  };
  ...
  const handleSheetsData = async () => {
    try {
      ...
      toast("داده‌ها در Google Sheets بارگذاری شد");
    } catch (error) {
      console.error("Error uploading data to Google Sheets:", error);
      toast("خطا در بارگذاری داده‌ها در Google Sheets");
    }
  };
  ...
  const handleExcel = () => {
    invoke("export_to_excel_command", {
      ...
      console.log(result);
      toast("خروجی داده‌ها به Excel گرفته شد");
    });
  };
