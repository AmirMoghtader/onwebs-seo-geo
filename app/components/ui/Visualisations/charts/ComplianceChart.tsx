...
export default function ComplianceChart({ crawlData }: VisualisationChartProps) {
  ...
  return (
    <div className="flex-1 flex items-center justify-around gap-2 flex-wrap">
      <CoverageRing label="امن با HTTPS" covered={stats.https} total={stats.total} />
      <CoverageRing label="داده ساختاریافته" covered={stats.schema} total={stats.total} />
      <CoverageRing label="سازگار با موبایل" covered={stats.mobile} total={stats.total} />
    </div>
  );
}
