...
const AIcontainer = () => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      toast.error("دریافت پاسخ از AI ناموفق بود");
    } finally {
      setIsThinking(false);
    }
  };
  ...
  const handlecopy = (content: string) => {
    navigator.clipboard.writeText(content)
      .then(() => toast("در کلیپ‌بورد کپی شد"))
  ...
  if (selectedModel !== "ollama" && selectedModel !== "gemini") {
    return (
      <div className="flex flex-col h-[70vh] items-center justify-center text-lg bg-gray-100 dark:bg-brand-darker border border-gray-300 dark:border-brand-dark rounded-lg">
        <span>هیچ مدل AI پیدا نشد.</span>
        <span className="block">
          برو به <strong>منو</strong> &gt; <strong>اتصال‌ها</strong>
        </span>
      </div>
    );
  }
