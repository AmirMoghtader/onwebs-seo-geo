...
export default function KeywordAnalytics() {
  ...
  const handleFullRefresh = useCallback(async () => {
    try {
      ...
      toast.success("داده‌ها بازخوانی و با GSC تطبیق داده شد");
    } catch (error) {
      console.error("Failed to refresh data:", error);
      toast.error("بازخوانی داده‌ها ناموفق بود");
    } finally {
      setIsLoading(false);
    }
  ...
  const removeKeyword = useCallback(
    async (id: string) => {
      try {
        ...
        toast.error("حذف Keyword ناموفق بود");
      }
    },
    [handleKeywordsSummary]
  );
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="bg-white dark:bg-brand-darker border-brand-dark">
                  <DropdownMenuItem onClick={() => invoke("match_tracked_with_gsc_command")}>
                    <Database className="mr-2 h-4 w-4" /> تطبیق اجباری با GSC
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
