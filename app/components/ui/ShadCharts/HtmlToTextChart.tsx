  ...
  return (
    <Card className="flex flex-col dark:bg-brand-darker dark:border-brand-darker h-[22rem]">
      <CardHeader className="items-center pb-0">
        <CardTitle>نسبت محتوا</CardTitle>
          ...
          <div className="flex flex-col items-center gap-2 font-medium leading-none -mt-10">
            {htmlToTextRatio?.[0] &&
              Math.round(htmlToTextRatio?.[0]?.[0] * 100) < 25 && (
                <>
                  <span>نسبت متن به HTML کم است.</span>
                  <span>این صفحه برای کسب رتبه بهتر به متن بیشتری نیاز دارد.</span>
                </>
              )}
            ...
            {htmlToTextRatio?.[0] &&
              Math.round(htmlToTextRatio?.[0]?.[0] * 100) > 25 && (
                <>
                  <span>نسبت متن به HTML شما خوب است.</span>
                </>
              )}
          </div>
        </CardFooter>
