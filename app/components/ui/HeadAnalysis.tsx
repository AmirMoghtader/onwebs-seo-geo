...
`}
            ...
            {favicon_url.length === 0 && pageTitle[0] && (
              <span className="text-red-500 -ml-3">هیچ favicon یافت نشد</span>
            )}
          </div>
          ...
          <div className="flex items-center mt-2">
            ...
            <div className="flex flex-wrap items-center">
              {hreflangs[0] === "No hreflang found" ? (
                <span className="ml-2">هیچ hreflang یافت نشد</span>
              ) : (
          ...
          <div className="flex items-center mt-2 mt-0 sm:mt-0 xl:mt-4">
            ...
            <span className="text-black ml-2">
              ...
              {openGraphDetails.image !== null && pageTitle?.length > 0 && (
                <span className="text-green-500">تگ OG در markup شما یافت شد</span>
              )}
            </span>
          </div>
...
`}
            ...
            <span className="text-black/80 ml-2">
              {pageSchema.length > 0 && pageTitle?.length > 0 && (
                <Link href="#sd">
                  <span className="text-green-500">داده ساختاریافته یافت شد</span>
                </Link>
              )}
              {!pageSchema[0] && pageTitle?.length > 0 && (
                <span className="text-red-500">پیدا نشد</span>
              )}
            </span>
          </div>
...
`}
            ...
            <span className="text-black/80 ml-2">
              ...
              {tagManager.length === 0 && pageTitle?.length > 0 && (
                <span className="text-red-500">container موجود نیست</span>
              )}
            </span>
          </div>
...
`}
            ...
            <span className="text-black/80 ml-2">
              ...
              {indexation.length === 0 && pageTitle?.length > 0 && (
                <span className="text-red-500">هیچ container یافت نشد</span>
              )}
            </span>
          </div>
        </aside>
