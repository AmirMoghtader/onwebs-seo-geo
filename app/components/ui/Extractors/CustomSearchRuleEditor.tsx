...
export default function CustomSearchRuleEditor({
  ...
  return (
    <div className="space-y-1">
      <SettingField label="نام قانون" description="A label to identify this rule in results">
        <Input
          value={rule.name}
          onChange={(e) => update({ name: e.target.value })}
          placeholder="مثلاً: دارای بنر کوکی"
          className="w-[220px] h-8 text-xs dark:bg-brand-darker dark:border-brand-dark dark:text-white"
        />
      </SettingField>

      <SettingField label="حالت" description="How the pattern is matched against the page">
        <Select
          value={rule.mode}
        ...
        >
          ...
          <SelectContent className="dark:bg-brand-darker dark:border-brand-dark dark:text-white">
            <SelectItem value="css" className="text-xs">سلکتور CSS</SelectItem>
      ...
      {rule.mode === "css" && (
        <>
          ...
          <SettingField label="چه چیزی ثبت شود" description="What to record from the matched element">
            <Select
              value={rule.target}
            ...
            >
              ...
              <SelectContent className="dark:bg-brand-darker dark:border-brand-dark dark:text-white">
                <SelectItem value="text" className="text-xs">متن</SelectItem>
          ...
          {rule.target === "attribute" && (
            <SettingField label="نام Attribute" description="e.g. href, src, content">
              <Input
      ...
      <SettingField label="فعال" description="Applies to every crawl while on">
