        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 dark:text-gray-200">بودجه روزانه</label>
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400">میانگین مبلغی که می‌خواهید هر روز خرج کنید.</p>
                </div>

                <div className="space-y-2 flex flex-col">
                    <label className="text-sm font-bold text-gray-700 dark:text-gray-200">استراتژی Bidding</label>
                    <select
                    ...
                    >
                        <option value="maximize_clicks">بیشینه‌سازی کلیک</option>
                        <option value="maximize_conversions">بیشینه‌سازی Conversion</option>
                        <option value="manual_cpc">CPC دستی</option>
            ...
            <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-white/5">
                <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 dark:text-gray-200">هدف‌گیری مکانی</label>
                    <input
                        value={(ad.locations || []).join(", ")}
                        onChange={(e) => onChange({ locations: e.target.value.split(",").map(s => s.trim()).filter(Boolean) })}
                        placeholder="مثلاً United States, London, Tokyo"
                ...
                <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 dark:text-gray-200">زبان‌ها</label>
                    <input
                        value={(ad.languages || []).join(", ")}
                        onChange={(e) => onChange({ languages: e.target.value.split(",").map(s => s.trim()).filter(Boolean) })}
                        placeholder="مثلاً English, Spanish, French"
                        className="w-full px-3 h-9 text-xs rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-brand-darker focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-medium placeholder:text-gray-400 text-gray-900 dark:text-gray-100"
                    />
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100 dark:border-white/5">
                <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 dark:text-gray-200">تاریخ شروع</label>
                    <input
                        type="date"
                        value={ad.startDate || ""}
                ...
                <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-700 dark:text-gray-200">تاریخ پایان</label>
                    <input
                        type="date"
                        value={ad.endDate || ""}
