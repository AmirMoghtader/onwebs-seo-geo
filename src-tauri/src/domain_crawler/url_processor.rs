...
pub async fn process_url(
    ...
    let (
        ...
        let page_meta_val = extract_page_meta(&document, &headers, &body, &http_version);
...
async fn update_state_and_emit_progress(
        ...
        && state.queue.len() < MAX_QUEUE_SIZE
    {
        ...
        for (normalized_url, url_pattern, normalized_url_obj) in prepared_links {
            ...
            // Configuration > Include / Exclude. Compiled once and cached by
            // pattern text, so this is a regex match rather than a rebuild.
            // List mode audits exactly the URLs it was handed; following links
            // out of them would turn it back into a spider crawl.
            if settings.list_mode {
                continue;
            }

            if !url_filters.allows(&normalized_url) {
                continue;
            }
