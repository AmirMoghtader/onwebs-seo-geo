    ...
    let api_key = match libs::load_api_keys().await {
        Ok(api_keys) => {
            if api_keys.page_speed_key.is_empty() {
                println!("No PageSpeed API key set — add one via Connectors > PageSpeed Insights");
                String::new()
            } else {
        ...
        Err(_) => {
            println!("Failed to load API keys file — add a key via Connectors > PageSpeed Insights");
            String::new()
        }
    };
